...
CREATE POLICY "Allow participants to insert conversations" ON public.conversations FOR INSERT WITH CHECK (true);

DROP POLICY IF EXISTS "Allow participants to update conversations" ON public.conversations;
CREATE POLICY "Allow participants to update conversations" ON public.conversations FOR UPDATE USING (auth.uid() = buyer_id OR auth.uid() = agent_id);
...
CREATE POLICY "Allow participants to insert messages" ON public.messages FOR INSERT WITH CHECK (
    EXISTS (
      SELECT 1 FROM public.conversations c
      WHERE c.id = public.messages.chat_id
        AND (c.buyer_id = auth.uid() OR c.agent_id = auth.uid())
...
DROP POLICY IF EXISTS "Allow sender to update own message status" ON public.messages;
CREATE POLICY "Allow sender to update own message status" ON public.messages FOR UPDATE USING (sender_id = auth.uid());

-- 4b. Admin policies — full read + message delete for moderation
DROP POLICY IF EXISTS "Allow admin to read all conversations" ON public.conversations;
CREATE POLICY "Allow admin to read all conversations" ON public.conversations FOR SELECT USING (public.is_admin());

DROP POLICY IF EXISTS "Allow admin to read all messages" ON public.messages;
CREATE POLICY "Allow admin to read all messages" ON public.messages FOR SELECT USING (public.is_admin());

DROP POLICY IF EXISTS "Allow admin to delete messages" ON public.messages;
CREATE POLICY "Allow admin to delete messages" ON public.messages FOR DELETE USING (public.is_admin());
...
CREATE INDEX IF NOT EXISTS idx_messages_chat_created ON public.messages USING btree (chat_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_conversations_buyer ON public.conversations USING btree (buyer_id, last_sent_at DESC);
CREATE INDEX IF NOT EXISTS idx_conversations_agent ON public.conversations USING btree (agent_id, last_sent_at DESC);
CREATE INDEX IF NOT EXISTS idx_conversations_last_sent ON public.conversations USING btree (last_sent_at DESC);
...
-- 7. Enable Realtime (safe — no explicit transaction block)
DO $$
BEGIN
  IF NOT EXISTS (
