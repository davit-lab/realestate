...
export default function CompareDrawer({
 ...
 <AnimatePresence>
  ...
  <>
   ...
   className="fixed bottom-0 left-0 right-0 z-[95] bg-white rounded-t-3xl shadow-2xl max-h-[85vh] flex flex-col"
   >
   {/* Header */}
   <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between shrink-0">
    ...
    <div>
     <h3 className="font-bold text-gray-900 text-sm">შედარება</h3>
    ...
    <button
     onClick={onClose}
     className="w-8 h-8 rounded-xl bg-gray-100 hover:bg-gray-200 :bg-gray-700 flex items-center justify-center transition-colors cursor-pointer"
    >
   ...
   <div className="flex-1 overflow-auto p-6">
    ...
    <div className="text-center py-12">
     <Scale size={32} className="mx-auto text-gray-200 mb-3" />
    ...
    <div className="overflow-x-auto">
     ...
     <thead>
      ...
      {listings.map(l => (
       ...
       <div className="relative group">
        <div
        className="aspect-[4/3] rounded-xl overflow-hidden bg-gray-100 mb-2 cursor-pointer"
        ...
        <p className="text-[12px] font-semibold text-gray-900 line-clamp-2 leading-snug cursor-pointer hover:text-ss-primary transition-colors"
        onClick={() => onViewDetail(l.id)}>
        {l.title}
        </p>
       </div>
     ...
     <tbody className="text-sm">
      ...
      <tr key={spec.key} className="border-t border-gray-100 ">
       ...
       {listings.map(l => (
       <td key={l.id} className="py-3 text-[13px] text-gray-800 font-semibold">
        {getValue(l, spec.key)}
       </td>
       ))}
      </tr>
