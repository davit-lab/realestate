...
export default function AdminPanel({ localListings, onDeleteListing }: Props) {
 ...
 const loadTickets = async () => { if(!isSupabaseConfigured)return; setLoading(l=>({...l,tickets:true})); const {data,error}=await supabase.from('support_tickets').select('*, profiles(name)').order('created_at',{ascending:false}).limit(100); if(error){console.error('[loadTickets] error:',error.message);} setTickets((data||[]).map((t:any)=>({...t,user_name:t.profiles?.name||t.name||'უცნობი'}))); setLoading(l=>({...l,tickets:false})); };
 ...
 useEffect(()=>{ if(sub==='users')loadUsers(); if(sub==='listings')loadDbListings(); if(sub==='support')loadTickets(); if(sub==='transactions')loadTransactions(); if(sub==='packages')loadPackages(); if(sub==='site')loadSettings(); if(sub==='templates')loadTemplates(); if(sub==='chats')loadChats(); },[sub]);
