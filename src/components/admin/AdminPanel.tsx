...
import {
 ...
 AlertTriangle, FileText, Settings, Moon, Sun, Shield, Banknote, Briefcase, BadgeCheck
} from 'lucide-react';
...
export default function AdminPanel({ localListings, onDeleteListing }: Props) {
 ...
 const loadChats = async () => { if(!isSupabaseConfigured){console.warn('[loadChats] Supabase not configured'); return;} setLoading(l=>({...l,chats:true})); const {data:convData,error:convError}=await supabase.from('conversations').select('*').order('last_sent_at',{ascending:false}).limit(100); console.log('[loadChats] convData:', convData?.length || 0, 'error:', convError?.message || 'none'); if(convError){console.error('[loadChats] error:',convError.message); setConversations([]);} else { const allIds=[...(convData||[]).map(c=>c.buyer_id).filter(Boolean),...(convData||[]).map(c=>c.agent_id).filter(Boolean)]; const uniqueIds=[...new Set(allIds)]; let profileMap=new Map(); if(uniqueIds.length>0){ const {data:profData}=await supabase.from('profiles').select('id, name, email').in('id',uniqueIds); console.log('[loadChats] profiles:', profData?.length || 0); (profData||[]).forEach((p:any)=>profileMap.set(p.id,p)); } const enriched=(convData||[]).map((c:any)=>({...c,buyer_name:profileMap.get(c.buyer_id)?.name||'უცნობი',agent_name:profileMap.get(c.agent_id)?.name||'უცნობი',buyer_email:profileMap.get(c.buyer_id)?.email||'',agent_email:profileMap.get(c.agent_id)?.email||''})); console.log('[loadChats] enriched conversations:', enriched.length); setConversations(enriched); } setLoading(l=>({...l,chats:false})); };
 ...
 const loadServices = async () => { if(!isSupabaseConfigured)return; setLoading(l=>({...l,services:true})); const {data:hData}=await supabase.from('hotels').select('*').order('created_at',{ascending:false}).limit(200); setHotels((hData||[]).map((h:any)=>({...h,pricePerNight:h.price_per_night,reviewCount:h.review_count}))); const {data:tData}=await supabase.from('tourism_items').select('*').order('created_at',{ascending:false}).limit(200); setTourismItems((tData||[]).map((t:any)=>({...t,reviewCount:t.review_count,date_info:t.date_info,time_info:t.time_info}))); setLoading(l=>({...l,services:false})); };
 const loadVerifications = async () => { if(!isSupabaseConfigured)return; setLoading(l=>({...l,verifications:true})); const {data,error}=await supabase.from('profile_verifications').select('*').order('submitted_at',{ascending:false}).limit(200); if(error){console.error('[loadVerifications] error:',error.message); setVerifications([]); setLoading(l=>({...l,verifications:false})); return;} const userIds=[...new Set((data||[]).map((v:any)=>v.user_id).filter(Boolean))]; let nameMap=new Map(); if(userIds.length>0){ const {data:profData}=await supabase.from('profiles').select('id,name').in('id',userIds); (profData||[]).forEach((p:any)=>nameMap.set(p.id,p.name)); } setVerifications((data||[]).map((v:any)=>({...v,user_name:nameMap.get(v.user_id)||'უცნობი'}))); setLoading(l=>({...l,verifications:false})); };

 useEffect(()=>{loadStats();},[localListings]);
 useEffect(()=>{ if(sub==='users')loadUsers(); if(sub==='listings')loadDbListings(); if(sub==='support')loadTickets(); if(sub==='transactions')loadTransactions(); if(sub==='packages')loadPackages(); if(sub==='site')loadSettings(); if(sub==='templates')loadTemplates(); if(sub==='chats')loadChats(); if(sub==='payments')loadPaymentProviders(); if(sub==='services')loadServices(); if(sub==='verifications')loadVerifications(); },[sub]);
 ...
 const toggleListingStatus = async (id:string,currentStatus:string)=>{ if(!isSupabaseConfigured)return; const newStatus=currentStatus==='live'?'inactive':'live'; await supabase.from('properties').update({status:newStatus}).eq('id',id); setDbListings(p=>p.map(l=>l.id===id?{...l,status:newStatus}:l)); _flash(newStatus==='live'?'განცხადება აქტიურია':'განცხადება გამორთულია',setFeedback); };
 ...
 const saveHotel = async (h:any)=>{ if(!isSupabaseConfigured)return; const payload:any={name:h.name,stars:h.stars,rating:h.rating,review_count:h.reviewCount||0,price_per_night:h.pricePerNight,city:h.city,district:h.district,image:h.image,images:h.images||[],amenities:h.amenities||[],description:h.description,phone:h.phone,tags:h.tags||[],featured:h.featured||false,updated_at:new Date().toISOString()}; if(h.id){ await supabase.from('hotels').update(payload).eq('id',h.id); setHotels(p=>p.map(x=>x.id===h.id?{...x,...h}:x)); } else { const {data:newRow}=await supabase.from('hotels').insert(payload).select().single(); if(newRow)setHotels(p=>[{...newRow,pricePerNight:newRow.price_per_night,reviewCount:newRow.review_count},...p]); } _flash('სასტუმრო შენახულია',setFeedback); };
 const deleteHotel = async (id:string)=>{ if(!isSupabaseConfigured)return; await supabase.from('hotels').delete().eq('id',id); setHotels(p=>p.filter(x=>x.id!==id)); _flash('სასტუმრო წაიშალა',setFeedback); };
 const toggleHotelActive = async (id:string,cur:boolean)=>{ if(!isSupabaseConfigured)return; await supabase.from('hotels').update({is_active:cur}).eq('id',id); setHotels(p=>p.map(x=>x.id===id?{...x,is_active:cur}:x)); _flash(cur?'სასტუმრო აქტიურია':'სასტუმრო გამორთულია',setFeedback); };
 const saveTourism = async (t:any)=>{ if(!isSupabaseConfigured)return; const payload:any={category:t.category,title:t.title,subtitle:t.subtitle,image:t.image,city:t.city,price:t.price,currency:t.currency||'₾',rating:t.rating,review_count:t.reviewCount||0,date_info:t.date_info||'',time_info:t.time_info||'',duration:t.duration||'',tags:t.tags||[],featured:t.featured||false,badge:t.badge||'',updated_at:new Date().toISOString()}; if(t.id){ await supabase.from('tourism_items').update(payload).eq('id',t.id); setTourismItems(p=>p.map(x=>x.id===t.id?{...x,...t}:x)); } else { const {data:newRow}=await supabase.from('tourism_items').insert(payload).select().single(); if(newRow)setTourismItems(p=>[{...newRow,reviewCount:newRow.review_count,date_info:newRow.date_info,time_info:newRow.time_info},...p]); } _flash('ტურისტული ობიექტი შენახულია',setFeedback); };
 const deleteTourism = async (id:string)=>{ if(!isSupabaseConfigured)return; await supabase.from('tourism_items').delete().eq('id',id); setTourismItems(p=>p.filter(x=>x.id!==id)); _flash('ტურისტული ობიექტი წაიშალა',setFeedback); };
 const toggleTourismActive = async (id:string,cur:boolean)=>{ if(!isSupabaseConfigured)return; await supabase.from('tourism_items').update({is_active:cur}).eq('id',id); setTourismItems(p=>p.map(x=>x.id===id?{...x,is_active:cur}:x)); _flash(cur?'ტურისტული ობიექტი აქტიურია':'ტურისტული ობიექტი გამორთულია',setFeedback); };
 ...
 const reviewVerification = async (vid:string, userId:string, status:'approved'|'rejected', note:string='')=>{ if(!isSupabaseConfigured)return; const updates:any={status,reviewed_at:new Date().toISOString(),reviewed_by:user?.id}; if(note) updates.admin_note=note; const {error}=await supabase.from('profile_verifications').update(updates).eq('id',vid); if(error){_flash('შეცდომა: '+error.message,setFeedback); return;} await supabase.from('profiles').update({is_verified:status==='approved',verification_status:status}).eq('id',userId); setVerifications(p=>p.map(v=>v.id===vid?{...v,...updates,is_verified:status==='approved',verification_status:status}:v)); _flash(status==='approved'?'ვერიფიკაცია დადასტურდა':'ვერიფიკაცია უარყოფილია',setFeedback); };

 const nav=[{id:'dashboard',label:'Dashboard',icon:LayoutDashboard},{id:'listings',label:'განცხადებები',icon:Home},{id:'users',label:'მომხმარებლები',icon:Users},{id:'transactions',label:'ტრანზაქციები',icon:CreditCard},{id:'packages',label:'VIP პაკეტები',icon:Crown},{id:'services',label:'სერვისები',icon:Briefcase},{id:'chats',label:'ჩატები',icon:MessageSquare},{id:'support',label:'საპორტი',icon:AlertTriangle},{id:'templates',label:'შაბლონები',icon:FileText},{id:'payments',label:'გადახდები',icon:Banknote},{id:'verifications',label:'ვერიფიკაციები',icon:BadgeCheck},{id:'site',label:'საიტის პარამეტრები',icon:Settings}];
 ...
 <div className={`min-h-screen w-full font-sans ${bgPage}`}>
  ...
  <main className="flex-1 p-5 overflow-auto">
   ...
   {sub==='dashboard'&&<DashboardTab stats={stats} localListings={localListings} dbListings={dbListings} onDeleteLocal={delDbListing} {...commonProps} />}
   {sub==='listings'&&<ListingsTab listings={[...dbListings,...localListings.filter(l=>!dbListings.find(d=>d.id===l.id))]} loading={loading.listings||false} onRefresh={loadDbListings} onDelete={delDbListing} onDeleteLocal={onDeleteListing} onToggleStatus={toggleListingStatus} {...commonProps} />}
   ...
   {sub==='services'&&<ServicesTab hotels={hotels} tourismItems={tourismItems} loading={loading.services||false} onRefresh={loadServices} onSaveHotel={saveHotel} onSaveTourism={saveTourism} onDeleteHotel={deleteHotel} onDeleteTourism={deleteTourism} onToggleHotel={toggleHotelActive} onToggleTourism={toggleTourismActive} {...commonProps} />}
   {sub==='verifications'&&<VerificationsTab verifications={verifications} loading={loading.verifications||false} onRefresh={loadVerifications} onReview={reviewVerification} {...commonProps} />}
