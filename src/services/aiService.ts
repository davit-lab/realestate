...
const SYSTEM_PROMPT = `შენ ხარ Newlife.ge-ის AI ასისტენტი — უძრავი ქონების პორტალის სპეციალისტი საქართველოში.
...
შენ ხარ Newlife-ს ნაწილი — არა ზოგადი AI. მოგვმართე ქართულად.`;

export function getApiKey(): string | null {
  return localStorage.getItem('newlife_kimi_key');
}

export function setApiKey(key: string) {
  localStorage.setItem('newlife_kimi_key', key);
}

export function removeApiKey() {
  localStorage.removeItem('newlife_kimi_key');
}
